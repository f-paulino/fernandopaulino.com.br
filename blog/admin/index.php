	...
	<?php
		try {
			...
			while($row = $stmt->fetch()){
				...
				echo '<tr>';
				echo '<td class="mdl-data-table__cell--non-numeric">&nbsp;&nbsp;&nbsp;'.$row['postTitle'].'</td>';
				echo '<td>'.date("d/m/Y", strtotime($row['postDate'])).'&nbsp; às &nbsp;'.date("H:i:s", strtotime($row['postDate'])).'</td>';
				?>

				<td style="text-align: right;">
					<a href="edit-post.php?id=<?php echo $row['postID'];?>&savedimg=<?php echo $row['postImg'];?>"><i class="material-icons">&#xE22B;</i></a>&nbsp;&nbsp;&nbsp;<a href="javascript:delpost('<?php echo $row['postID'];?>','<?php echo $row['postTitle'];?>')"><i class="material-icons">&#xE872;</i></a>
				</td>
